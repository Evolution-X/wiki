...
PATH = "/content/evolution-settings"
...
def main():
    ...
    for xml_file in XML_FILES:
        ...
        if xml_content:
            ...
            if content:
                filename = xml_file.replace(".xml", ".mdx").replace("evolution_settings_", "")
    ...
    # Create index file
    index_path = os.path.join(PATH, "index.mdx")
    with open(index_path, "w", encoding="utf-8") as f:
        ...
        for category, filename in sorted(categories.items()):
            f.write(f"- [{category}](/evolution-settings/{filename})\n")
