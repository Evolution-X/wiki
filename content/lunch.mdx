...
<Callout type="error">
  **23/02/2024** **FROM NOW ON, FOR ANDROID 15, YOU NEED TO DO `lunch
  lineage_$device-ap4a-$build_type`.**
</Callout>
...
So commands will be in the form <LunchCommand>lunch lineage\_$device-$release-$build_type</LunchCommand>.
...
<Callout type="warning">
  ...
  git knowledge is required. Besides, make sure the disk you will work on is
  formatted with **Ext4**.
</Callout>
...
<Callout type="info">
  ...
  [here](https://github.com/Evolution-X/manifest/blob/udc/snippets/evolution.xml).
</Callout>
