...
Since 08/2024, we support vanilla builds (without gapps) but it's up to the maintainer. Be careful and download the
correct package !

---

To get started, you will need to download the ROM for your device. It is important to download the correct build for
your device.
...
- [<img src="/favicons/sfav.png" alt="sitefav" style="width:20px; vertical-align:middle;"> EvolutionX's Mirror](https://evolution-x.org/) (
...
After having downloaded the correct ROM build and vendor/firmware zip file, we can proceed with the flashing process.
Boot your device into recovery mode, and wipe data, cache and system using our recovery.
...
EvolutionX has a built-in OTA update system. This means that when an update is out, it'll come directly in your settings
with a notification, like if it was an "official" update (like if OnePlus or Xiaomi deployed one)
...
*Here, we assume you have [adb drivers](https://www.xda-developers.com/install-adb-windows-macos-linux/) installed on
your computer.*
...
Follow the instructions displayed on the screen, depending on the device, it could propose you to update the recovery at
the same time.
...
If you want to do so, put the zip file on your sd card, and select this build after clicking "Update from internal
storage"
...
Only a few roms (GrapheneOS for example) and only a few devices (some Pixels, and some OnePlus) are able to do this
